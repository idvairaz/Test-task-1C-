...
## Написание кода проведения

### Выполненные действия:

**1. Написан код проведения для документа "Приобретение товаров"**
   - При проведении формирует движения по регистру `InventoryBalances` (Приход)

**2. Написан код проведения для документа "Реализация товаров"**
   - При проведении:
     - Рассчитывает среднюю себестоимость
     - Формирует движения по регистру `InventoryBalances` (Расход)
     - Формирует движения по регистру `SalesRevenue` (Обороты)
...
**1. Создан отчет "Отчет по складским остаткам"**
   - Источник данных: регистр `InventoryBalances`
   - Показывает остатки товаров на складах

**2. Создан отчет "Отчет по продажам с наценкой"**
   - Источник данных: регистр `SalesRevenue`
   - Показывает продажи и рассчитанную наценку
...
**Документы:**
- Приобретение №1: Ноутбук (10 шт × 50 000), Мышь (20 шт × 1 500)
- Приобретение №2: Ноутбук (5 шт × 55 000), Клавиатура (15 шт × 800)
- Реализация №1: Ноутбук (7 шт × 65 000), Мышь (10 шт × 2 000)

### Результаты:

**Средняя себестоимость:**
- Ноутбук: (10 × 50 000 + 5 × 55 000) / 15 = 51 666.67
- Мышь: 1 500
- Клавиатура: 800
...
|-------|------------|-------|
| Ноутбук | 8 | 413 333.31 |
| Мышь | 10 | 15 000 |
| Клавиатура | 15 | 12 000 |

**Продажи:**
| Товар | Выручка | Себестоимость | Наценка |
|-------|---------|---------------|---------|
| Ноутбук | 455 000 | 361 666.69 | 25.8% |
| Мышь | 20 000 | 15 000 | 33.3% |
